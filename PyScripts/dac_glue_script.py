import sys
import dateutil.parser
# from datetime import datetime
from itertools import chain
from pyspark.sql import functions as F
from awsglue.transforms import *
from awsglue.utils import getResolvedOptions
from pyspark.context import SparkContext
from awsglue.context import GlueContext
from pyspark.sql.types import IntegerType, DoubleType, StringType
from pyspark.sql.functions import (
    col, when, coalesce, regexp_extract, 
    udf, to_date, round, trim, lower, year, lit, create_map
)

# ✅ Get the input file name from Lambda trigger
args = getResolvedOptions(sys.argv, ['PATH'])
path = args['PATH']
 # Use dynamic file path

# Initialize AWS Glue Job
sc = SparkContext()
glueContext = GlueContext(sc)
spark = glueContext.spark_session

# Read CSV from S3 using the dynamically passed file name
df = spark.read.option("header", "true").csv(path)

# Print Column Names and Data Types (for Debugging)
print("Initial Data Types:")
for col_name, col_type in df.dtypes:
    print(f"Column : {col_name}, Data Type : {col_type}")

# ------------------------------------------------------
# Data Type Conversions
# ------------------------------------------------------

# Identify Columns to Convert to IntegerType
int_columns = [col_name for col_name in df.columns if "_Theory" in col_name or 
               "_Lab" in col_name or "_Total" in col_name or col_name == "Total"]

# Convert Selected Columns to IntegerType
for col_name in int_columns:
    df = df.withColumn(col_name, df[col_name].cast(IntegerType()))

# Convert 'Percentage' and 'pg_percentage' Columns to DoubleType
df = df.withColumn("Percentage", df["Percentage"].cast(DoubleType()))
df = df.withColumn("pg_percentage", df["pg_percentage"].cast(DoubleType()))

# ------------------------------------------------------
#  Cleaning 'CCATrank' Column
# ------------------------------------------------------

df = df.withColumn("CCATrank", regexp_extract(col("CCATrank"), r"(\d+)", 1))
# Explicitly casting 'CCATrank' to IntegerType
df = df.withColumn("CCATrank", df["CCATrank"].cast(IntegerType()))

# ------------------------------------------------------
#  Cleaning 'DOB' Column
# ------------------------------------------------------

# Function to normalize date formats
def parse_date(date_str):
    if date_str:
        try:
            return dateutil.parser.parse(date_str).strftime('%Y-%m-%d')
        except Exception:
            return None
    return None

# Register as a UDF
parse_date_udf = udf(parse_date, StringType())

# Manually fix specific incorrect dates before applying the parser
df = df.withColumn(
    "dob",
    when(col("dob") == "14 januray 1997", "14 January 1997")
    .when(col("dob") == "6 Jully 1997", "6 July 1997")
    .otherwise(col("dob"))
)

# Apply the UDF to normalize date formats
df = df.withColumn("dob", parse_date_udf(col("dob")))

# ------------------------------------------------------
# Processing Percentage Columns
# ------------------------------------------------------

def process_percentage_column(df, column_name):
    df = df.withColumn(column_name, regexp_extract(col(column_name), r"(\d*\.?\d+)", 1).cast("double"))

    # Apply transformations for CGPA and percentage adjustments, then round to 2 decimal places
    df = df.withColumn(
        column_name,
        round(
            when((col(column_name) > 0) & (col(column_name) < 1.0), col(column_name) * 100)  # Convert 0.x to percentage
            .when((col(column_name) >= 1.0) & (col(column_name) < 10.0), col(column_name) * 9.5)  # Convert CGPA to %
            .otherwise(col(column_name)),  # Keep other values unchanged
            2  # Round to 2 decimal places
        )
    )
    return df

# Example usage
df = process_percentage_column(df, "10th_percentage")
df = process_percentage_column(df, "grad_percentage")
df = process_percentage_column(df, "12th_percentage")
df = process_percentage_column(df, "diploma_percentage")
df = process_percentage_column(df, "pg_percentage")
df = process_percentage_column(df, "Percentage")

# ------------------------------------------------------
#  Processing 'Higher_Edu_Percent' Column
# ------------------------------------------------------

df = df.withColumn(
    "Higher_Edu_Percent",
    when(col("12th_percentage").isNotNull() & col("diploma_percentage").isNotNull(), 
         (col("12th_percentage") + col("diploma_percentage")) / 2)  # Take the average if both are available
    .otherwise(coalesce(col("12th_percentage"), col("diploma_percentage")))  # Else take the non-null value
)

# Dropping the old columns
df = df.drop("12th_percentage", "diploma_percentage")

df = process_percentage_column(df, "Higher_Edu_Percent")

# ------------------------------------------------------
# Processing 'pre_ccat' Column
# ------------------------------------------------------

df = df.withColumn("pre_ccat", trim(lower(col("pre_ccat"))))
df = df.withColumn("pre_ccat", when(col("pre_ccat") == "yes", "yes").otherwise("no"))

# ------------------------------------------------------
# Processing 'Age' Column
# ------------------------------------------------------

df = df.withColumn("dob", to_date(col("dob"), "yyyy-MM-dd"))
# Calculate age by subtracting the year extracted from 'dob' from the 'year' column
df = df.withColumn("Age", col("year") - year(col("dob")))

# ------------------------------------------------------
# Processing 'Branch' Column
# ------------------------------------------------------

def extract_branch(column):
    return (
        when(lower(col(column)).rlike("computer|information technology|it|i.t"), "Computer")
        .when(lower(col(column)).rlike("mechanical|mech"), "Mechanical")
        .when(lower(col(column)).rlike("electronics|telecommunication|e&tc|extc|e and tc|entc"), "Electronics and Telecommunication")
        .when(lower(col(column)).rlike("civil"), "Civil")
        .when(lower(col(column)).rlike("math"), "Mathematics")
        .when(lower(col(column)).rlike("phys"), "Physics")
        .when(lower(col(column)).rlike("chem"), "Chemical")
        .when(lower(col(column)).rlike("instru"), "Instrumentation")
        .when(lower(col(column)).rlike("electrical"), "Electrical")
        .when(lower(col(column)).rlike("bsc"), "BSc")
        .otherwise("BE")  # Categorize remaining values as "Other"
    )

# Apply extraction logic on both columns
df = df.withColumn(
    "branch_cleaned",
    when(col("branch").isNull() | (lower(col("branch")) == "other"), extract_branch("grad_degree"))  
    .otherwise(extract_branch("branch"))  
)

# Dropping unwanted columns
df = df.drop("branch", "grad_degree")

# ------------------------------------------------------
# Handling 2 Columns Project Grade,Aptitude Grade
# ------------------------------------------------------

df = df.withColumn(
    "Project_Grade",
    when(lower(col("Project_Grade")).rlike("absent|ab"), None)  # Convert 'absent' to NULL
    .when(lower(col("Project_Grade")).rlike("fail"), "F")  # Convert 'fail' to 'F'
    .otherwise(col("Project_Grade"))  # Keep other values unchanged
)

df = df.withColumn(
    "Apti_EC_Grade",
    when(lower(col("Apti_EC_Grade")).rlike("absent|ab"), None)  # Convert 'absent' to NULL
    .when(lower(col("Apti_EC_Grade")).rlike("fail"), "F")  # Convert 'fail' to 'F'
    .otherwise(col("Apti_EC_Grade"))  # Keep other values unchanged
)
# -------------------------------------------------------------------------------------------
# Extracting LPA from Company_Name
# -------------------------------------------------------------------------------------------
package_regex = r"(\d+\.?\d*\s*[Ll][Pp][Aa])"  # Extracts "8.5 LPA", "7 lpa", case insensitive
company_cleanup_regex = r"[-.,\s]*\d+\.?\d*\s*[Ll][Pp][Aa]"  # Removes extracted package & extra -,.

# Extract Package value only when "LPA" (case insensitive) exists
df = df.withColumn(
    "Package",
    F.when(
        F.col("Company_Name").rlike("(?i)LPA"),  # Only extract if "LPA" exists
        F.regexp_extract(F.col("Company_Name"), package_regex, 1)
    ).otherwise(F.col("Package"))  # Keep existing values
)

# Clean Company_Name by removing extracted LPA values and trailing -,., spaces
df = df.withColumn(
    "Company_Name",
    F.regexp_replace(F.col("Company_Name"), company_cleanup_regex, "").alias("Company_Name")
).withColumn(
    "Company_Name",
    F.regexp_replace(F.col("Company_Name"), r"[-.,\s]+$", "")  # Remove any remaining -, . , at the end
).withColumn(
    "Company_Name",
    F.regexp_replace(F.col("Company_Name"), r"^\s+|\s+$", "")  # Trim spaces from start and end
)
# ------------------------------------------------------
# ------------------------------------------------------
#Reducing Redundancy in Company_Name Column
# ------------------------------------------------------

company_mapping = {
    "A.K.Capital": ["A.k. Capital Services", "AK.Capital"],
    "Active AI": ["Active.Ai"],
    "Adenza": ["ADENZA"],
    "Adform": ["Adform India LLP"],
    "Amazatic Solutions": ["Amazatic Solution"],
    "Avati": ["Avati Consulting Solutions (P) Ltd.", "Avati", "Avati Consulting Solutions (P) Ltd"],
    "Avenue Ecommerce": ["Avenue Ecommerce (DMart India)", "Avenue Ecommerce (DMart India"],
    "Bajaj": ["Bajaj Finserv"],
    "BakerHughes": ['Baker Hughe', "BAKER HUGHES"],
    "Bill Desk": ["BILL DESK", "BillDesk", "BILLDESK"],
    "Cdac Mumbai": ['CDAC MUMBAI', 'CDAC Mumbai'],
    "Collinson": ["collinson Group"],
    "Crimson": ["CRIMSON INTERACTIVE", 'CRIMSON', 'CRIMSON8'],
    "CCtech": ['CCTEch'],
    "Cybage": ['cybage'],
    "Crisil": ["CRISIL BRIS", "CRISIL Ltd. (Client of 3I Infotech)", 'CRISIL', 'CRISIL Ltd. (Client of 3i Infotech)'],
    "Dassault Systems": ["Dassault Systemes"],
    "Ebix": ["Ebixcash Financial Technologies", 'Ebixcash Financial Technologie'],
    "Emphasys": ['EeMPHASYS', 'eEmphasy'],
    "Hitachi Payments": ["HITACHI PAYMENT"],
    "Herald Logic": ['Herald Logic'],
    "IDBI": ["IDBI In Tech"],
    "IKS": ["IKS HEALTH"],
    "Itiviti": ["Itiviti a Broadridge Business", 'Itiviti a Broadridge Busine', 'ITIVITI'],
    "Infosys": ['Infosy'],
    "Knight Fintech": ["KFin Technologies Limited", "Knight FinTech Research Pvt Ltd", "KFINTECH", 'KNIGHT FINTECH', 'KNIGHT Fintech', 'KFintech'],
    "Laminar": ["LAMINAAR AVIATION"],
    "LearningMate": ["LearningMate"],
    "Loyalty Rewardz": ["Loylty Rewardz Mgmt Pvt Ltd"],
    "MorningStar": ["Morningstar India Pvt Ltd", "Morningstar, Inc.", "MORNINGSTAR", 'MORNING STAR', 'Morning Star', 'Morningstar, Inc'],
    "Motifworks India Private Limited": ["Motifworks Motifworks India Private Limited"],
    "Neiron": ["Neiron India"],
    "Nomura": ['nomura'],
    "Neosoft": ['NeoSoft', 'neoSoft', 'neosoft'],
    "NSE": ["NSE INDIA"],
    "Pelican": ["Pelican AI", 'PELICAN'],
    "Peocit": ['peocit', 'Peocit software'],
    "Publicis Sapient Digital": ['Publicis SapientDigita'],
    "RBHU": ["RBHU Analytics"],
    "RBL": ["RBL Bank"],
    "Reliance Jio": ["Reliance Jio Platforms", 'RELIANCE JIO', 'Reliance Jio Platform'],
    "Sapiens": ["Sapiens Technologies India Pvt. Ltd.", 'SAPIENS', 'Sapiens Technologies India Pvt. Ltd'],
    "Smartstream": ["SmartStream Technologies", 'SMARTSTREAM', 'SmartStream Technologie'],
    "Square Yards": ["SQUAREYARDS"],
    "Tech Mahindra": ["Tech Mahindraahindra", 'TECH M'],
    "TIAA": ["TIAA GBS Pvt. Ltd.", 'TIAA GBS Pvt. Ltd'],
    "Unplaced": ["Unplaceded"]
}
# Convert mapping dictionary to a PySpark expression
mapping_expr = create_map(
    *list(chain(*[(lit(v), lit(k)) for k, v_list in company_mapping.items() for v in v_list]))
)
# Replace values in Company_Name based on mapping
df = df.withColumn("Company_Name", coalesce(mapping_expr[col("Company_Name")], col("Company_Name")))

# ------------------------------------------------------
#  Processing 'Is_Placed' Column
# ------------------------------------------------------

# Define values that indicate the candidate is NOT placed
not_placed_values = ["fail", "30 calls done", "failed", "opted out for placement", "out of placements"]
# Create the 'Is_Placed' column with case-insensitive matching
df = df.withColumn(
    "Is_Placed",
    when(
        (col("Company_Name").isNull()) & (col("Package").isNull()), "No"  # If both Company_Name & Package are NULL → No
    )
    .when(
        (col("Company_Name").isNull()) & (col("Package").isNotNull()), "Yes"  # If Company_Name is NULL but Package exists → Yes
    )
    .when(
        lower(trim(col("Company_Name"))).isin(not_placed_values), "No"  # Case-insensitive check for non-placement values
    )
    .otherwise("Yes")  # Otherwise, if Company_Name exists → Yes
)

# ------------------------------------------------------
# 🔹 Saving Cleaned Data to S3
# ------------------------------------------------------

output_path = "s3://outputdac/dac"
df.write.mode("overwrite").option("header", "true").csv(output_path)

# Print Confirmation
print(f"✅ Data successfully transformed and saved to: {output_path}")
